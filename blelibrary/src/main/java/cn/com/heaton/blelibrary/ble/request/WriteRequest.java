...
public class WriteRequest<T extends BleDevice> implements WriteWrapperCallback<T> {
    ...
    public boolean writeStrByUuid(T device, String data, UUID serviceUUID, UUID characteristicUUID, BleWriteCallback<T> callback){
        this.bleWriteCallback = callback;
        boolean result = false;
        BleRequestImpl bleRequest = BleRequestImpl.getBleRequest();
        if (bleRequest != null) {
            result = bleRequest.wirteCharacteristicStrByUuid(device.getBleAddress(), data, serviceUUID, characteristicUUID);
        }
        return result;
    }
